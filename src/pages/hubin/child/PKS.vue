...
const response_data = computed(() => store.getters.mou)
onMounted(() => {
    store.dispatch("getMou")
})

const selectedMou = ref()
const toast = useToast();
const onRowSelect = (event) => {
    toast.add({ severity: 'info', summary: 'Dudi Selected', detail: 'Name: ' + event.data.nama, life: 3000 });
    cek.value = false
};
const onRowUnselect = (event) => {
    toast.add({ severity: 'warn', summary: 'Dudi Unselected', detail: 'Name: ' + event.data.name, life: 3000 });
    cek.value = true
}

const ubah = () => {
    console.log(selectedMou.value)
}
const cek = ref(true)
const dt = ref();
const exportCSV = () => {
...
<template>
    ...
    <div class="card">
        <DataTable v-model:selection="selectedMou" :value="response_data.rows" selectionMode="single" dataKey="mou_id"
            :metaKeySelection="false" @rowSelect="onRowSelect" @rowUnselect="onRowUnselect"
            tableStyle="min-width: 50rem, max-height: 50rem" ref="dt">
            <Column field="nama_dudi" header="Jenis Kerja Sama" class="text-sm"></Column>
            <Column field="nama_dudi" header="Nama Dudi" class="text-sm">
            </Column>
            <Column field="nomor_mou" header="No.PKS" class="text-sm"></Column>
            <Column field="judul_mou" header="Judul PKS" class="text-sm"></Column>
            <Column field="tanggal_mulai" header="Tgl.Mulai" class="text-sm"></Column>
            <Column field="tanggal_selesai" header="Tgl.Selesai" class="text-sm"></Column>
            <Column field="nama_bidang_usaha" header="Nama Bidang Usaha" class="text-sm"></Column>
        </DataTable>
        <Toast />
    </div>
