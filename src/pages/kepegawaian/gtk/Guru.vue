...
onMounted(() => {
    store.dispatch("getGtk", 'guru')
})

const selectedGtk = ref()
const toast = useToast();
const onRowSelect = (event) => {
    toast.add({ severity: 'info', summary: 'GTK Selected', detail: 'Name: ' + event.data.nama, life: 3000 });
    cek.value = false
};
const onRowUnselect = (event) => {
    toast.add({ severity: 'warn', summary: 'GTK Unselected', detail: 'Name: ' + event.data.name, life: 3000 });
    cek.value = true
}
...
<template>
    <div class="flex justify-between">
        <div class="space-x-2">
            ...
            <Button label="Profil Guru" :disabled="cek" />
